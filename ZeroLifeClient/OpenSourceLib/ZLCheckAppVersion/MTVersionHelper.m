...
-(void)versionRequest:(CheckVersionBlock)newVersion
{
    [self getversionRequestSuccess:^(NSDictionary *responseDict) {
        ...
        if(resultCount==1)
        {
            ...
            appInfo.trackViewUrl = [self replaceStr:appInfo.trackViewUrl];
...
/**
 将appstore美国区us替换为cn中国区

 @param willReplaceStr 要替换的字符串
 @return 返回替换后的字符串
 */
- (NSString *)replaceStr:(NSString *)willReplaceStr{

    return [willReplaceStr stringByReplacingOccurrencesOfString:@"us" withString:@"cn"];
}
@end
