...
- (void)UpdateSpeViewPage:(NSString *)mGoodsImg andGoodsName:(NSString *)mName andGoodsPrice:(float)mPrice andSkuCount:(int)mcount andGoodsNum:(int)mNum{
    ...
    if (price<_mShopObj.mShopMsg.ext_min_price) {
        mbtnContent = [NSString stringWithFormat:@"还差%.2f元起送",_mShopObj.mShopMsg.ext_min_price-price];
        mSpeView.mBuyNowBtn.userInteractionEnabled = NO;
        [mSpeView.mBuyNowBtn setBackgroundColor:[UIColor lightGrayColor]];
    }else if (price<=0){
        mbtnContent = [NSString stringWithFormat:@"还差%.2f元起送",_mShopObj.mShopMsg.ext_min_price];
        mSpeView.mBuyNowBtn.userInteractionEnabled = NO;
        [mSpeView.mBuyNowBtn setBackgroundColor:[UIColor lightGrayColor]];
    }else{
        mbtnContent = @"去结算";
...
- (void)ZLSuperMarketAddBtnSelected:(NSIndexPath *)mIndexPath{
    ...
    for (ZLSpeObj *mObj  in self.mAddSkuArray) {
        if (mObj.mSku.sta_required == 1) {
            ...
            [self UpdateSpeViewPage:_mClsGoodsObj.img_url andGoodsName:_mClsGoodsObj.pro_name andGoodsPrice:mObj.mSku.sku_price andSkuCount:mObj.mSku.sku_stock andGoodsNum:_mClsGoodsObj.mNum];
