...
-(void)loadAPIwithState:(NSString *)stateStr orderItem:(OrderObject *)item isShopOrderBool:(BOOL)isShopOrderBool call:( void(^)(OrderObject *itemNew))callback
{
    ...
    if (isShopOrderBool)
    {
        if (item.odr_type == kOrderClassType_paopao) {
            
            if ([stateStr isEqualToString:kOrderState_DIFFWAIT]) //提交差价
            {
    ...
    else
    {
        ...
        {
            OrderRepairBidObject *bidItem = [self bk_associatedValueForKey:USER_FIX_CHOOSE_BID_Key];
            ...
            if (bidItem != nil) {
                ...
                vc.chooseCallBack = ^(OrderRepairBidObject* item111){
                    ...
                    [SVProgressHUD showWithStatus:@"操作中..."];
                    [[APIClient sharedClient] orderOprateBidWithTag:self odr_id:item.odr_id odr_code:item.odr_code bid_id:item111.bid_id call:^(ZLCreateOrderObj *payItem, APIObject *info) {
                        if (info.code == RESP_STATUS_YES) {
